...
public class RedisUtils {
    ...
    public Boolean execut(String script, List<String> keys, Object... args) {
        // 构造RedisScript对象,泛型为返回值类型,第一个参数为脚本字符串,第二个参数为泛型的字节码类型
        final DefaultRedisScript<Long> redisScript = new DefaultRedisScript<>(script, Long.class);
        try {
            // 调用execute方法执行脚本,keys为脚本需要的key,args为key的value值
            final Long num = this.redisTemplate.execute(redisScript, keys, args);
            if (num == 1) {
                return true;
            }
            return false;
     ...
     */
    public boolean setNXDistributedLock(String key, Object value) {
        try {
            final Boolean ifAbsent = this.redisTemplate.opsForValue().setIfAbsent(key, value, 300, TimeUnit.SECONDS);
            return ifAbsent;
        } catch (Exception e) {
            e.printStackTrace();
            return false;
    ...
    public void zIncrementScore(String key, Object value, long delta) {
        this.redisTemplate.opsForZSet().incrementScore(key, value, delta);
    }

    public void zUnionAndStore(String key, Collection otherKeys, String destKey) {
        this.redisTemplate.opsForZSet().unionAndStore(key, otherKeys, destKey);
    }
    ...
    public long batchZSet(String key, Set<ZSetOperations.TypedTuple<Object>> typles) {
        return this.redisTemplate.opsForZSet().add(key, typles);
    }
